...
import { feedPosts, InsertFeedPost } from "../drizzle/schema";

const mockPosts: InsertFeedPost[] = [
  // Twitter Mock Posts
...
async function seed() {
  ...
  console.log("Seeding mock posts to the database...");
  
  for (const post of mockPosts) {
    try {
      await db.insert(feedPosts).values(post).onDuplicateKeyUpdate({
        set: post
      });
      console.log(`✅ Inserted/Updated post: ${post.externalId} from ${post.platform}`);
    } catch (err) {
      console.error(`❌ Failed to insert ${post.externalId}:`, err);
    }
  }
